# -*- coding: utf-8 -*-
"""
Created on Fri Oct 17 12:24:50 2018
...
test_df['title'] = test_df['title'].apply(lambda x: punc.sub('',x))

#tokenization
test_df['title'] = test_df['title'].apply(lambda x: nltk.word_tokenize( x ))

# Remove stop words from term vectors
stop_words = nltk.corpus.stopwords.words( 'english' )

test_df['title'] = test_df['title'].apply(lambda x: ([word for word in x if word not in stop_words]))
...
#     test_df.loc[index,'title'] = term_list
#     count += 1
#     print(count)

stem_df = test_df.copy()
